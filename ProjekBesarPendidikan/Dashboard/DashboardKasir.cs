...
namespace ProjekBesarPendidikan.Dashboard
{
    public partial class DashboardKasir : Form
    {
        ...
        public DashboardKasir(DashboardAdmin dashboardAdmin)
        {
            ...
            PieChart("select * from dbo.fn_Top5PlaystationPalingSeringDisewa(" + tahun + ", " + bulan + ")", "Top5");
            PieChart("select * from dbo.fn_Top5MetodePembayaranPalingSeringDipakai(" + tahun + ", " + bulan + ")", "Jenis");
            label();
        }

        //ngisi 2 kotak di atas
        public void label()
        {
            SqlCommand cmd;
            SqlDataAdapter da;
        ...
        private void RefreshPieCharts()
        {
            ...
            Top5PS.Series.Clear();
            ListPenyewaanPerbulan.Series.Clear();
        ...
        //kebetulan sama jumlah kolom yang ada di data makanya di jadikan satu saja dan di bedakan di bagian mycmd ama jenis pie nya
        public void PieChart(String mycmd, string jenis)
        {
            SqlCommand cmd;
            SqlDataAdapter da;
            DataSet ds;
            ...
            da.Fill(ds, "Motor");
            dt = ds.Tables["Motor"];
            Series series = new Series("Data") { ChartType = SeriesChartType.Pie };

            // Looping melalui semua baris dalam tabel
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                string nama = dt.Rows[i][0].ToString(); // Mengambil nama kategori
                double jumlah = Convert.ToDouble(dt.Rows[i][2]); // Mengambil nilai jumlah
            ...
            if (jenis.Equals("Top5")){
                if (dt.Rows.Count == 0)
                {
                    Top5PS.Titles[0].Visible = false;
                }
                Top5PS.Series.Add(series);
            }else{
                if (dt.Rows.Count == 0)
                {
                    ListPenyewaanPerbulan.Titles[0].Visible = false;
                }
                ListPenyewaanPerbulan.Series.Add(series);
            }
            conn.Close();
        }
        ...
        private void ListPenyewaanPerbulan_Click(object sender, EventArgs e)
        {
