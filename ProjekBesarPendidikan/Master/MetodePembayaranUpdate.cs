...
namespace ProjekBesarPendidikan.Master {
    public partial class MetodePembayaranUpdate : Form {
        ...
        private void btn_save_Click(object sender, EventArgs e) {
            ...
            try {
                using (SqlConnection conn = new SqlConnection(Connection)) {
                    using (SqlCommand cmd = new SqlCommand("rps_updateMetodePembayaran", conn)) {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@mpb_id", id);
                        cmd.Parameters.AddWithValue("@mpb_nama", nama);
                        cmd.Parameters.AddWithValue("@mpb_deskripsi", deskripsi);
                        cmd.Parameters.AddWithValue("@mpb_modif_by", createdBy);
